...
function wc_predictive_install(){
	...
	update_option('wc_predictive_search_version', '2.3.5');
	update_option('wc_predictive_search_plugin', 'woo_predictive_search');
	...
	*/
	if(isset($_POST['wc_predictive_pin_submit'])){
		...
		if($respone_api == md5('valid')) {
			update_option( 'a3rev_pin_woo_predictive_search', sha1(md5('a3rev.com_'.get_option('siteurl').'_woo_predictive_search')));
...
function woo_predictive_search_check_pin() {
	...
	if (get_option('a3rev_auth_woo_predictive_search') != '' && get_option("a3rev_pin_woo_predictive_search") == sha1(md5('a3rev.com_'.$domain_name.'_woo_predictive_search'))) return true;
	else return false;
}
